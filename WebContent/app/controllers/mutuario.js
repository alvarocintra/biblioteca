...
APP.controller('MutuarioDeletarController', function($scope, $state, $stateParams, AutorService) {	
  $scope.autor = MutuarioService.remove({ id: $stateParams.id });
  
  $scope.deletarMutuario = function(){
	$scope.mutuario.$deletar(function(){
		$state.go('mutuarios',{'sucessMessage': "Mutuario deletado com sucesso!"});
	}, function(){
		$scope.errorMessage = "Ocorreu um erro no servidor. Verifique se todos os campos foram preenchidos corretamente.";
	}
	);  
